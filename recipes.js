...
const createRecipe = Recipe.create({
  title: "Tortila de patatas con cebolla",
  cuisine: "hola"
})
  .then(result => {
...
const updateOne = Recipe.findByIdAndUpdate("5bc07ad4095b07858b3e5ea0", {
  duration: 100
})
  .then(result => {
    console.log("updated");
...
const removeOne = Recipe.findOneAndRemove({ title: "Carrot Cake" })
  .then(result => {
    console.log("borrado");
  })
  .catch(error => {
...
Promise.all([createRecipe, insertMany, updateOne, removeOne])
  .then(result => {
