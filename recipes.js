  ...
  .then(() => {
    ...
    Recipe.insertMany([
      ...
      .then((rcp) => {
        ...
        const dbo = db('recipeApp');
        dbo.recipes.update({ title: 'Rigatoni alla Genovese' }, { $set: { duration: 100 } });
        dbo.recipes.remove({ title: 'Carrot Cake' });
      })

      .catch((err) => {
        console.log('Error in recipe creation', err);
      });
  }).catch((err) => {
