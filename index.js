...
mongoose
  ...
  async function allFunctions(){
    const createOne = await Recipe.create(receta1)
    ...
    const createArray = await Recipe.insertMany(recipesArr)
      .then(newRecipes => {
    ...
    const updateOne = await Recipe.findOneAndUpdate(rigatoni, { duration: 100 })
      .then(recipe => console.log("A recipe has been updated"))
      .catch(err => console.log(`Error while updating a recipe: ${err}`));

    const deleteOne = await Recipe.deleteOne({title: "Carrot Cake"})
      .then(recipe => console.log("A recipe has been deleted"))
      .catch(err => console.log(`Error while deleting a recipe: ${err}`));  
