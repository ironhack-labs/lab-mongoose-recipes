...
mongoose
  ...
  .then(() => {
    ...
    // add one recipe
    const data1 = { title: 'Pasta with cheese', level: 'Easy Peasy', ingredients:['tomato', 'meat'], cuisine: 'indian',dishType:'main_course', duration: 1, creator: 'M. Issa'};
    Recipe.create(data1)
  ...
  .then(()=> {
    // update one recipe
    Recipe.findOneAndUpdate({title:'Rigatoni alla Genovese'}, {duration: 100 }, function(err, doc){
      if(err){
