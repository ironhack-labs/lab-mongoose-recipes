...
mongoose
  ...
  .then(() => {
    const carbonara = { 
      title: "Carbonara", 
      level: 'Amateur Chef',
      ingredients: ['spaghetti', 'eggs', 'pancetta', 'pecorino cheese', 'salt', 'black pepper'],
      cuisine: "Italian", 
      dishType: "main_course",
      image: 'https://cdn.panelinha.com.br/receita/1565893815418-carbonara.jpg',
      duration: 20,
      creator: 'Carolina'
    }
  ...
  .then(newRecipe => console.log('Nova Receita criada', newRecipe))
  .then(() => {
    return Recipe.findOneAndUpdate({ title: 'Rigatoni alla Genovese'}, {duration: 20}, {new: true})
  }) 
  .then(updatedRecipe => console.log('Receita atualizada', updatedRecipe))
  .then(() => {
    return Recipe.deleteOne({ title: 'Carrot Cake'})
  }) 
  .then(deletedRecipe => console.log('Receita deletada', deletedRecipe))
  .catch(error => {
    console.error('Error connecting to the database', error);
  });

  mongoose.connection.on('connected', () => console.log("Conexão aberta")) 
  mongoose.connection.on('error', (error)  => console.log("Erro na conexão", err))
